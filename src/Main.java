...
public class Main extends Application {
    ...
    public void start(Stage stg) throws Exception {
        ...
        Text gameText = new Text("You can purchase upgrades on this screen.\n Press START to attempt a run.\nPressing the back button will automatically create a save file.");
        Text engineUpgradeText = new Text("Upgrade your engine to increase your speed.\nPress the button to purchase an upgrade.");
        Text NOSPurchaseText = new Text("Purchase illegal nitrogen (NOS) to rapidly increase your speed. \n(Adds ability to boost speed for a moment. press E to use)\nPress the button to purchase an upgrade.");
        Text WheelUpgradeText = new Text("Upgrade your wheels to increase your ability to not be destroyed by potholes\nPress the button to purchase an upgrade.");
        ...
        menuItem.setOnAction(value -> {
            ...
            if (selectedFile != null) {
                ...
                } else {InputHandler.setTaxi(temp);}
            }
        });
        ...
        main_box.getChildren().addAll(mb, titleText, selectedFile_Text, start_btn, continue_btn, manual_btn, difficulty_btn,
                exit_btn);
        ...
        back_game_btn.setOnAction(value -> {
            ...
            {
                if (InputHandler.getTaxi() == null) {stg.setScene(menu_scene); return;}
                Alert noSave = new Alert(Alert.AlertType.INFORMATION, "No save file selected.\nWould you like to create a new save file?\nIf no file is selected, the game will not be saved.");
        ...
        refresh_stats_button.setOnAction(value -> {
            if (InputHandler.getTaxi() == null) {return;}
            walletInfo.setText(String.format("Cash avaliable: %.2f", InputHandler.getTaxi().getWallet()));
            passengerInfo.setText(String.format("Total Passengers picked up: %d", InputHandler.getTaxi().getCareerPassengers()));
        });
        ...
        start_btn.setOnAction(value -> {
            ...
            {
                Alert alert = new Alert(Alert.AlertType.CONFIRMATION,"You already have a game started, would you like to start a new game?\nWARNING: This will overwrite your current save.");
